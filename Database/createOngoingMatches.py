...
def assign_round_3_game_3_stats_to_players(conn, round_id, game_id, home_team_id, away_team_id):
    ...
    calculate_round_3_game_3_quarter_score(conn, 1, round_id, game_id, home_team_id, home_score)
    away_score = add_round_3_game_3_player_stats_per_team(conn, away_team_id, round_id, game_id)
    calculate_round_3_game_3_quarter_score(conn, 1, round_id, game_id, away_team_id, away_score)
...
def add_round_3_game_3_player_stats_per_team(conn, team_id, round_id, game_id):
    ...
    team_score = 0 
    for p in team_players:
        player = Player(p[0],random.randint(15, 30))
        player_points_scored_so_far = int((player.freethrows_in+2*player.two_points_in+3*player.three_points_in)/1.5)
        if team_score>50:
            player_points_scored_so_far = int(player_points_scored_so_far/2.5)
        elif team_score>30:
            player_points_scored_so_far = int(player_points_scored_so_far/1.5)
        ...
        q1_three_points_in = random.randint(0, 1)*random.randint(0, int(q1_points_copy/3)) if q1_points_copy>=3 else 0
        q1_points_copy -= 3*q1_three_points_in
        q1_two_points_in = random.randint(0, int(q1_points_copy/2)) if q1_points_copy>=2 else 0
        q1_points_copy -= 2*q1_two_points_in
...
def calculate_round_3_game_3_quarter_score(conn, championship_id, round_id, game_id, team_id, total_score):
    db_cursor = conn.cursor()
    score_copy = total_score
    
    avg_quarter_score = int(score_copy/2)
    q1_score = random.randint(avg_quarter_score-3,avg_quarter_score+5)
    q2_score = score_copy-q1_score
    db_cursor.execute("INSERT INTO team_score_per_quarter (championship_id, round_id, game_id, quarter, team_id, quarter_score) VALUES (%s, %s, %s, %s, %s, %s)", (championship_id, round_id, game_id, 1, team_id, q1_score))
    db_cursor.execute("INSERT INTO team_score_per_quarter (championship_id, round_id, game_id, quarter, team_id, quarter_score) VALUES (%s, %s, %s, %s, %s, %s)", (championship_id, round_id, game_id, 2, team_id, q2_score))
...
for game in round_3_games:
    ...
    if game_id<3:
        ...
        assign_round_3_game_3_stats_to_players(conn,game[0],game[1],game[2],game[3])
